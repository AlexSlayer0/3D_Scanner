...
- PointCloud als numpy-Array (keine Objekt-Attribute)
- Dynamische Referenzhöhe aus Tiefenbild-Messung
...
class Config:
    # Keine feste Referenz mehr – wird dynamisch ermittelt
    MIN_OBJEKT_HOEHE_MM = 10.0      # Mindesthöhe über Boden
    MAX_OBJEKT_HOEHE_MM = 500.0     # Maximalhöhe
    MAX_LAENGE_BREITE_MM = 800.0    # Maximales Objekt
    
    POINTCLOUD_SPARSE = True        # Sparsam = schnell, reicht für BoundingBox
    ...
    def get_measurement(self) -> Dict:
        try:
            ...
            with dai.Device(pipeline, maxUsbSpeed=dai.UsbSpeed.HIGH) as device:
                ...
                # Dynamische Referenzhöhe: Median des gesamten Tiefenbildes (Boden)
                referenz_mm = float(np.median(valid_depths))
                ...
                if np.sum(valid_mask) == 0:
                    return self._error_result("Keine gültigen Z-Werte")
                
                x_valid = x_coords[valid_mask]
                y_valid = y_coords[valid_mask]
                z_valid = z_coords[valid_mask]
                
                logger.info(f"📊 Punktwolke: {len(z_valid)} gültige Punkte")
                ...
                # ===== OBJEKT OBERHALB BODEN? =====
                # Wenn alle Z-Werte größer als Referenz + Toleranz → Boden frei / kein Objekt
                if min_z > referenz_mm + 20:
                    return self._error_result(f"Kein Objekt erkannt (min_z {min_z:.1f} > Referenz {referenz_mm:.1f})")
                ...
                # Höhe = Referenz - min_z (Objekthöhe über Boden)
...
if __name__ == "__main__":
    ...
    print("\n🚀 Starte 3D-Volumenmessung (dynamische Referenz, USB2)\n")
    result = get_volume()
