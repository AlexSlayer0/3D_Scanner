...
KAMERA_ABSTAND_REFERENZ_MM = 550
...
OBJEKT_ABSTAND_DELTA_MM = 5  # Objekt muss mind. 10mm über Referenz liegen
...
def pixel_zu_mm(ox, oy, ow, oh, roi_x1, roi_y1, roi_w_px, roi_h_px, tiefe_mm):
    """
    Umrechnung von Pixelmaßen in mm, unter Berücksichtigung der Position in der ROI
    ox, oy : Pixelkoordinaten innerhalb des Depth-ROIs
    ow, oh : Pixelbreite / -höhe des Objekts
    roi_x1, roi_y1 : Startpixel der ROI
    roi_w_px, roi_h_px : Größe der ROI in Pixel
    tiefe_mm : Höhe des Objekts über Referenzfläche (zur Skalierung)
    """
    # mm/px auf Referenzhöhe
    scale_x = MESSBEREICH_BREITE_MM / roi_w_px
    scale_y = MESSBEREICH_HOEHE_MM / roi_h_px

    # optional: anpassen für andere Höhe
    scale_x *= tiefe_mm / KAMERA_ABSTAND_REFERENZ_MM
    scale_y *= tiefe_mm / KAMERA_ABSTAND_REFERENZ_MM

    breite_mm = ow * scale_x
    laenge_mm = oh * scale_y
...
with pipeline:
    ...
    while pipeline.isRunning():
        ...
        if objekt:
            ...
            breite_mm, laenge_mm = pixel_zu_mm(
                ox - x1,  # Objektposition relativ zur ROI
                oy - y1,
                ow, oh,
                x1, y1,
                roi_w, roi_h, 
